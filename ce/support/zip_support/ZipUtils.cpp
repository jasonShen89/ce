...
inline void ZipUtils::ccDecodeEncodedPvr(unsigned int *data, int len)
{
    ...
    CCAssert(s_uEncryptedPvrKeyParts[0] != 0, "CE: CCZ file is encrypted but key part 0 is not set. Did you call ZipUtils::ccSetPvrEncryptionKeyPart(...)?");
    CCAssert(s_uEncryptedPvrKeyParts[1] != 0, "CE: CCZ file is encrypted but key part 1 is not set. Did you call ZipUtils::ccSetPvrEncryptionKeyPart(...)?");
    CCAssert(s_uEncryptedPvrKeyParts[2] != 0, "CE: CCZ file is encrypted but key part 2 is not set. Did you call ZipUtils::ccSetPvrEncryptionKeyPart(...)?");
    CCAssert(s_uEncryptedPvrKeyParts[3] != 0, "CE: CCZ file is encrypted but key part 3 is not set. Did you call ZipUtils::ccSetPvrEncryptionKeyPart(...)?");
...
int ZipUtils::ccInflateCCZFile(const char *path, unsigned char **out)
{
    ...
    compressed = CCFileUtils::getInstance()->getFileData(path, "rb", &fileLen);
...
#if (CC_TARGET_PLATFORM != CC_PLATFORM_WINRT) && (CC_TARGET_PLATFORM != CC_PLATFORM_WP8)
static pthread_mutex_t s_asynctt;
#endif
